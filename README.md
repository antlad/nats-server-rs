...
This repository is fully generated by localy served model (nvidia/Qwen3.8-Flash-Next-NVFP4)[https://huggingface.co/nvidia/Qwen3.8-Flash-Next-NVFP4] . 
It was implemented just only for experiment how far we can go with local served LLM.
...
crates/bench      pubsub / latency / fanout / floodpub / floodpubsub bench binaries
...
Faster than the reference on both paths a NATS Core server has, measured as CPU
seconds per message with the two binaries driven inside the same rounds (the
protocol `specs/perf-notes.md` explains; a single unpaired round on this host is
worth nothing).

| | reference | this server | ours / reference |
|---|---:|---:|---:|
| publish-only, CPU µs per message | 0.298 | **0.265** | **0.89×** |
| delivery, CPU µs per delivered message | 1.215 | **0.734** | **0.60×** |
| fan-out ×50, CPU µs per delivery | 0.760 | **0.507** | **0.67×** |
| allocations per published message | 0.751 | **0.010** | 0.013× |
| allocations per delivered message | 1.059 | **0.111** | 0.10× |
| `writev` / `futex` per delivered message | 0.0039 / 0.014 | **0.0011 / 0.00057** | 0.28× / 0.04× |
| memory held for a wedged subscriber | 1.98× `max_pending` | **1.00×** | 0.50× |

Raw runs: `benchmarks/raw/2026-09-22-pubonly-final.txt`,
`benchmarks/raw/2026-09-22-pubsub-final.txt` (three-arm against this build, four
rounds each; `0.60×` is the median of per-round ratios, which spanned 0.43–0.89 as
the machine changed regime mid-session);
per-site allocation census in `benchmarks/alloc-census.md`, regenerated by
`./benchmarks/tools/census.sh`.

Zero allocations per message is not literally what the two rows say: they say it on
the publish path to within a factor of one hundred, and on the delivery path to
within a factor of ten. What the remaining 0.111 is — an 8 KiB frame chunk per 29
messages, the refcount plumbing that comes with the chunk, a channel block per 32
queued frames — is attributed per site in `benchmarks/alloc-census.md`, and none of
it is per message. `specs/perf-notes.md` has the
attribution and the two bugs that were hiding behind code that looked correct:
`BytesMut`'s `BufMut::remaining_mut` answers `usize::MAX - len`, so every "is there
room" test written with it is always yes.
